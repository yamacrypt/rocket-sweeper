...
using VRC.Udon;
    ...
    public class IntKeyChunkOperationDictionary : UdonSharpBehaviour
    {
        int[][] _keys;
        ChunkOperation[][] _values;
        int _size;
        ...
        public void SetCapacity(int capacity)
        {
            ...
            _values=new ChunkOperation[len][];
            for(int i=0;i<_values.Length;i++){
                _values[i]=new ChunkOperation[bucketCount];
            }
        ...
        public bool Add(int key, ChunkOperation value)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
            ...
            ExpandList(index);
            //Debug.LogWarning("InkEyChunkOperationArrDictionary Add: Dictionary is full");
        ...
        public void AddOrSetValue(int key,ChunkOperation value){
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
                if(key==int.MaxValue){
            ...
            var preLen=hashKeys.Length;
            ExpandList(index);
            //Debug.LogWarning("InkEyChunkOperationArrDictionary AddOrSet: Dictionary is full");
            _keys[index][preLen]=key;
            _values[index][preLen]=value;
            _size++;
        }

        public void SetValue(int key, ChunkOperation value)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
        ...
        public ChunkOperation GetValueOrDefault(int key, ChunkOperation defaultValue){
            int index = Hash(key);
            var hashKeys=_keys[index];
            var hashValues=_values[index];
            if(index==0){
        ...
        public ChunkOperation GetValue(int key)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            var hashValues=_values[index];
