...
using VRC.Udon;
    ...
    public class IntKeyChunkStateDictionary : UdonSharpBehaviour
    {
        int[][] _keys;
        ChunkState[][] _values;
        int _size;
        ...
        public void SetCapacity(int capacity)
        {
            ...
            _values=new ChunkState[len][];
            for(int i=0;i<_values.Length;i++){
                _values[i]=new ChunkState[bucketCount];
            }
        ...
        public bool Add(int key, ChunkState value)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
            ...
            ExpandList(index);
            //Debug.LogWarning("InkEyChunkStateArrDictionary Add: Dictionary is full");
        ...
        public void AddOrSetValue(int key,ChunkState value){
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
                if(key==int.MaxValue){
            ...
            var preLen=hashKeys.Length;
            ExpandList(index);
            //Debug.LogWarning("InkEyChunkStateArrDictionary AddOrSet: Dictionary is full");
            _keys[index][preLen]=key;
            _values[index][preLen]=value;
            _size++;
        }

        public void SetValue(int key, ChunkState value)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
        ...
        public bool IsLoaded(int key){
            var state=GetValueOrDefault(key,ChunkState.None);
            return state==ChunkState.Detailed || state==ChunkState.UnDetailed;
        }
        public bool IsDetailed(int key){
            var state=GetValueOrDefault(key,ChunkState.None);
            return state==ChunkState.Detailed;
        }
        public bool IsUnDetailed(int key){
            var state=GetValueOrDefault(key,ChunkState.None);
            return state==ChunkState.UnDetailed;
        }
        public bool IsUnLoaded(int key){
            var state=GetValueOrDefault(key,ChunkState.None);
            return state==ChunkState.UnLoaded;
        }
        public ChunkState GetValueOrDefault(int key, ChunkState defaultValue){
            int index = Hash(key);
            var hashKeys=_keys[index];
            var hashValues=_values[index];
            if(index==0){
        ...
        public ChunkState GetValue(int key)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            var hashValues=_values[index];
