...
using VRC.Udon;
    ...
    public class IntKeyBiomeArrDictionary : UdonSharpBehaviour
    {
        int[][] _keys;
        Biome[][][] _values;
        int _size;
        ...
        public void SetCapacity(int capacity)
        {
            ...
            _values=new Biome[len][][];
            for(int i=0;i<_values.Length;i++){
                _values[i]=new Biome[bucketCount][];
            }
        ...
        public bool Add(int key, Biome[] value)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
        ...
        public void AddOrSetValue(int key,Biome[] value){
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
                if(key==int.MaxValue){
            ...
            var preLen=hashKeys.Length;
            ExpandList(index);
            //Debug.LogWarning("InkEyBiomeArrDictionary AddOrSet: Dictionary is full");
            _keys[index][preLen]=key;
            _values[index][preLen]=value;
            _size++;
        }

        public void SetValue(int key, Biome[] value)
        {
            int index = Hash(key);
            var hashKeys=_keys[index];
            if(index==0){
